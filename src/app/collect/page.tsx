...
export default function CollectPage() {
    ...
    try {
      ...
      try {
        ...
        if (parsedResult.wasteTypeMatch && parsedResult.quantityMatch && parsedResult.confidence > 0.7) {
          ...
          if (reward !== null) {
            console.log(`The reward is ${reward}`); // For the error of 'reward'
          }        
  ...
  //     // Update the task with the earned reward
  //     setTasks(tasks.map(task => task.id === selectedTask.id ? { ...task, reward: earnedReward } : task))
  //   } else {
  //     toast.error('Verification failed. The collected waste does not match the reported waste.', {
  //       duration: 5000,
  //       position: 'top-center',
